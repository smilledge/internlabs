...
module.exports = function() {
   ...
   */
  var loadTemplate = function(template, scope, callback) {
    ...
    fs.readFile(process.env.PWD + '/' + templatesDir + template, "utf8", function(err, data) {
      var body = ejs.compile(data);
      callback(err, body(scope));
    });
  }
      ...
      async.series([
        ...
        function(next) {
          if (options.template) {
            ...
            loadTemplate(template, options, function(err, template) {
              if (err) {
                return callback(err);
              }
